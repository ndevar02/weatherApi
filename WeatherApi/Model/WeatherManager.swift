...
struct WeatherManager {
    ...
    public func getWeather(cityname : String) {
        let urlstring = "\(url)&q=\(cityname)"
        performWeather(urlString: urlstring)
    }
    
    
    private func performWeather(urlString : String) {
    ...
    private func parsejson(weatherData: Data) -> CityWeather? {
        
        let decoder = JSONDecoder()
        do{
            let value = try decoder.decode(WeatherData.self, from: weatherData)
            let cityWeather = CityWeather(name: value.name, sunrise:
                                            Util.formatDate(inputString: value.sys.sunrise), sunset:Util.formatDate(inputString: value.sys.sunset), temperature: value.main.temp, description: value.weather[0].description)
            return cityWeather
